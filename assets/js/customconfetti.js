/////--- Initialization ---/////
// console.log('customconfetti.js loaded');
$(window).on("load", () => {
    ...
    ///colors
    var backgroundColor = "rgb(255 255 255 / 0%)";
    ...
    function removeConfettiById(id) {
        for (var i = 0; i < confettis.length; i++) {
            var c = confettis[i];
            if (c.id == id) {
                VX.removePoint(c.pt1.id);
                VX.removePoint(c.pt2.id);
                VX.removePoint(c.pt3.id);
                VX.removePoint(c.pt4.id);
                VX.removeSpan(c.sp1.id);
                VX.removeSpan(c.sp2.id);
                VX.removeSpan(c.sp3.id);
                VX.removeSpan(c.sp4.id);
